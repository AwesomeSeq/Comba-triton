...
class Mamba3(nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.num_heads = (hidden_size * self.expand_v) // 64

        self.value_dim = int(hidden_size * self.expand_v)
        ...
        self.q_proj = nn.Linear(hidden_size, 128, bias=False)
        self.k_proj = nn.Linear(hidden_size, 128, bias=False)
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        # q,k 从 (b,t,d) 复制 (b,t,h,d)
        q = q[:, :, None, :].expand(-1, -1, self.num_heads, -1).contiguous()
        k = k[:, :, None, :].expand(-1, -1, self.num_heads, -1).contiguous()
