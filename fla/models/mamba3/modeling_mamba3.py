...
class Mamba3Model(Mamba3PreTrainedModel):
    ...
    def __init__(self, config: Mamba3Config):
        ...
        self.layers = nn.ModuleList([Mamba3Block(config, layer_idx) for layer_idx in range(config.num_hidden_layers)])
    ...
    def forward(
        ...
        **kwargs: Unpack[Dict]
        ...
        if output_attentions:
            warnings.warn("`Mamba3Model` does not `output_attentions` now, setting it to `False`.")
...
class Mamba3ForCausalLM(Mamba3PreTrainedModel, GenerationMixin):
