...
def chunk_bwd_kernel_dqkwg(
    ...
    if USE_G:
        ...
        # S = exp(g_last) * S + v_new * exp(g_last - g_i)k
        ...
        if USE_DW:
            ...
            p_dw = tl.make_block_ptr(dw, (T, K), (s_qk, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            tl.store(p_dw, -b_dw.to(p_dw.dtype.element_ty), boundary_check=(0, 1))
        # b_dq: d (exp(g) * q), d(exp(g))= exp(g)
        # b_dg = b_dq * b_q * exp(g) = (b_dq * exp(g)) * b_q = b_dq(new) * b_q
        b_dq = b_dq * exp(b_g)[:, None] * scale
        b_dg += tl.sum(b_dq * b_q, axis=1)
        # b_dk: d (exp(g_last - g) * k)
        # b_d(g_last - g) = b_dk * b_k * exp(g_last - g) = (b_dk * exp(g_last - g)) * b_k = b_dk(new) * b_k
        b_dk = b_dk * safe_exp(-b_g + b_g_last)[:, None]
        b_dg -= tl.sum(b_k * b_dk, axis=1)
        b_dg_last += tl.sum(b_dk * b_k)

        # ds = d (QK exp(gi-gj))
        # ds_1 = d(QK) = ds * exp(gi - gj)
        # d(gi-gj) = ds * d(QK) * exp(gi-gj) = ds_1 * d(QK)
