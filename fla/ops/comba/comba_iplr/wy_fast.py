...
def fwd_prepare_wy_repr_kernel_chunk64(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        ...
        b_Aw += tl.dot(b_pb, tl.trans(b_k))
        b_Aw2 += tl.dot(b_pb2, tl.trans(b_k2))
        b_Aw3 += tl.dot(b_pb2, tl.trans(b_k))
...
def bwd_prepare_wy_repr_kernel(
    ...
    for i_k in range(tl.cdiv(K, BK)):
        if HEAD_FIRST:
            p_p = tl.make_block_ptr(p + i_bh * T*K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
            p_dp = tl.make_block_ptr(dp + i_bh * T*K, (T, K), (K, 1), (i_t * BT, i_k * BK), (BT, BK), (1, 0))
        ...
        b_dA += tl.dot(b_dw, tl.trans(b_p_beta), allow_tf32=False)
        b_dp_beta = tl.dot(b_A, b_dw, allow_tf32=False)
    ...
    if HEAD_FIRST:
        ...
        p_g = tl.make_block_ptr(g + (bos*H + i_h), (T,), (H,), (i_t * BT,), (BT,), (0,))
    b_g = tl.load(p_g, boundary_check=(0,))
    # dA2 = d (Diag(beta)PK) = d (exp(gi-gj) * Diag(beta)PK) * exp(gi-gj), single one
    b_dA2 *= safe_exp(b_g[:, None] - b_g[None, :])
    # b_dA: two of d (Diag(beta)PK)
    b_dA += b_dA2
